...
class Orchestrator:
    ...
    def _plan(self, query: str) -> dict:
        ...
        _STANDALONE_SITES = (
            ...
            (("수강신청 방법", "수강신청 어떻게", "수강 신청 어떻게", "수강신청 대리",
              "수강신청 전화", "수강신청 절차", "수강정정 방법", "수강정정 어떻게",
              "수강 정정 방법"),
             "수강신청 안내(학사지원과)",
             "https://plus.cnu.ac.kr/html/kr/sub05/sub05_05030201.html"),
        ...
        if cal:
            ...
            _n = _now(); _dow = ("월", "화", "수", "목", "금", "토", "일")[_n.weekday()]
            prompt = (
                f"[오늘 날짜] {_n:%Y년 %m월 %d일} {_dow}요일 — 요일은 '{_dow}요일'을 그대로 쓰고 "
                "다시 계산하지 마세요. '며칠 남음/몇 주차/몇 달 남음' 등 기간 계산이 필요하면 "
                "반드시 이 날짜를 기준으로 계산하고, 자료에 없으면 추측하지 마세요.\n"
                ...
                "계산하세요.\n"
                "6) [미공지 일정 추정 금지] 질문이 묻는 일정(예: 다음 학기 등록금 납부 기간)이 "
                "본문에 명시돼 있지 않으면 '아직 학사일정에 공지되지 않았습니다 — 확정 일정은 "
                "학사일정 페이지에서 확인해 주세요'로 답하세요. 개강일 등 다른 날짜에서 "
                "'며칠/몇 주 후일 것'이라 추정하거나 '예상됩니다/일반적으로 N주일' 같은 추측 표현을 "
                "절대 쓰지 마세요. 본문에 있는 일정만 단정합니다.\n\n"
                f"[학사일정 캘린더]\n{page_text}\n\n[질문]\n{query}"
            )
        ...
        if ir.intent == Intent.CAFETERIA:
            # 운영시간·영업여부 질의는 메뉴 덤프가 아니라 운영 안내로 위임(메뉴 요청과 구분).
            if (_re.search(r"운영\s*시간|영업\s*시간|몇\s*시|영업\s*(?:하|해|되|중|여|일|을)|"
                           r"문\s*(?:여|닫)|주말.*영업|주말.*(?:하나요|운영|되나요)", query)
                    and not _re.search(r"메뉴|식단|뭐\s*나|뭐\s*먹|먹을|반찬|식단표", query)):
                return P(Intent.CAFETERIA, is_fallback=True, refined=refined, static=(
                    "학생식당 운영시간·영업일은 식당별·학기별(방학 중 단축, 주말 휴무 등)로 "
                    "달라집니다. 정확한 운영시간은 아래 '식당 운영' 안내를 확인해 주세요:\n"
                    "https://plus.cnu.ac.kr/html/kr/sub05/sub05_05050101.html\n\n"
                    "메뉴가 궁금하시면 '오늘 학식 뭐 나와?'처럼 날짜를 넣어 물어봐 주세요."))
    ...
    def _read_academic_calendar(self, query: str):
        ...
        if not _re.search(
                ...
                # 휴학/계절학기 신청 '일정'은 학년도마다 갱신 → 인덱스 stale(2024) 회피 위해
                # 라이브 학사일정으로. 규정질의('휴학 최대 몇 학기')는 '신청' 없어 안 걸림.
                r"휴학\s*신청|복학\s*신청|계절\s*학기\s*(?:신청|일정|기간|등록|수강)|"

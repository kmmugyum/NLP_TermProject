...
> **[2026-06-09 19:41 갱신 — 와이드 크롤 완주 + 인덱스 빌드 + eval 통과]**
> 아래 본문(6,642청크)은 부분크롤 시점 기록. 이후 79개 학과 와이드 크롤(crawl_wide.py)을
> 완주(79/79, 3,313쪽, 2,036초)하여 커버리지를 보강하고 전체 재청킹했다.
>
> | 지표 | 최종 v2 (와이드크롤 반영) |
> |---|---|
> | 청크 수 | **11,425** (raw 12,713 → dedup/noise drop) |
> | 완전 중복 | **0** |
> | 빈 파이프 표 | **0** |
> | <200자 | **0** |
> | 평균 길이 | 984자 |
...
>
> **인덱스 빌드**: KURE-v1 cuda:1, ok=true, 11,425 vec, embed_dim 1024, 466초 → `academic_v2.bin` 46.8MB.
> **eval(OLD 23,642 vs NEW 11,425)**: 미적분학 학년·1학년 교과목 질문이 OLD=⚠깨짐 → NEW=[ok]로
> 학년-과목 매핑 깨끗하게 개선 확인. 상세는 `HANDOFF_REBUILD_DONE.md`.
> 라이브 `academic_real.bin`은 미덮어씀(스테이징 academic_v2.bin로만 생성).
